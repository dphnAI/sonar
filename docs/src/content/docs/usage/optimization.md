...
Larger offloaded chunks reduce per-block bookkeeping overhead but increase the granularity of lookups. When self-describing KV events are enabled, chunk mode (`block_size` greater than the GPU block size or `blocks_per_chunk` greater than `1`) can re-announce shared per-block hashes for overlapping chunks, so external consumers should reference-count repeated store and remove announcements.
