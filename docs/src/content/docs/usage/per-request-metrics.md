...
```bash
aphrodite run meta-llama/Llama-3.1-8B-Instruct --enable-per-request-metrics
```
...
:::note
Timing metrics describe a single generation stream, so they are only returned
when the request maps to exactly one. They are suppressed for requests with
`n > 1`, because the underlying timing data reflects only one of the `n`
sequences and cannot be accurately attributed to the request as a whole. Token
usage remains accurate in these cases. Per-request metrics also require
server-side statistics logging, which is on by default. Aphrodite rejects
`--enable-per-request-metrics` when `--disable-log-stats` is also set.
:::
