...
// 2-SM (cta_group::2) MMA. Tile-M (the weight N dimension) spans the two
// CTAs of an SM pair, 128 columns each. Tile-N 256 because per-instruction
// issue overhead caps a 256x128 UMMA at about two thirds of the 256x256
// rate, measured as a flat throughput ceiling across every other sweep.
...
using TiledMma = decltype(cute::make_tiled_mma(
    cute::SM100_MMA_F16BF16_2x1SM_SS<MmaType, MmaType, ElementAccumulator,
        cute::size<0>(MmaTileShape{}), cute::size<1>(MmaTileShape{}),
...
// Pipeline stage counts, derived from the smem budget below.
constexpr int kSmemCapacity = cutlass::gemm::collective::detail::sm100_smem_capacity_bytes;
constexpr int kKernelCarveout = 2048;
constexpr int kEpilogueBytes = int(sizeof(typename CollectiveEpilogue::SharedStorage));
// TMEM is 512 columns and an N=256 accumulator needs 256 per stage.
constexpr int kAccumStages = 2;
// B stage is the CTA's half of the N-split activation tile, 128x128 bf16.
constexpr int kInputStageBytes = 8192 + 32768 + 256 + 64;
constexpr int kComputeStageBytes = 32768 + 32;
constexpr int kT2MStages = 2;
constexpr int kAvail = kSmemCapacity - kKernelCarveout - kEpilogueBytes
                     - kAccumStages * 32 - kT2MStages * kComputeStageBytes;
constexpr int kL2TStages = kAvail / kInputStageBytes < 4 ? kAvail / kInputStageBytes : 4;
...
using CollectiveMainloop = cutlass::gemm::collective::SwordfishMainloopSm100MixedInput<
    ...
    // across the CTA pair and the leader's MMA reads the peer's half, so the
    // copy must deliver both halves with one arrival on the leader's
