   ...
   public:
    ...
    void apply_softmax(logits_buffer_t* __restrict__ logits_buffer,
      ...
      for (int32_t i = 0; i < q_head_num; ++i) {
        ...
        // FP8 probabilities are scaled to the maximum finite value, so
        // they must remain <= 1. Other paths retain the FA4 threshold.
        ...
        bool need_rescale =
            rescale_factor < (prequantize_probabilities ? 0.0f : -8.0f);
        if (!need_rescale) {
