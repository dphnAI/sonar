...
class MiMoVLProcessor:
    ...
    Media strings (URL / path / data:) are resolved upstream in Aphrodite's
    media pipeline; this processor accepts only already-decoded inputs
    (``PIL.Image`` for images, ``(waveform, sr)`` tuples for audio).
    """
    ...
    def preprocess_audio(self, audio: Any) -> tuple[torch.Tensor, int]:
        """Convert a pre-loaded ``(waveform, sr)`` tuple into mel features."""
        if not isinstance(audio, tuple):
            raise ValueError(
                ...
                "resolution is handled upstream in Aphrodite's media pipeline."
            )
...
class MiMoOmniProcessor(ProcessorMixin):
    ...
    and audio ((waveform, sr) tuple / numpy array).
    """
        ...
        **kwargs: Any,
        ...
        Args:
            ...
            videos: list of ``(frames_TCHW: torch.Tensor, timestamps_T: torch.Tensor)``
                    tuples (pre-decoded).
            audio: list of ``(waveform_1D, sample_rate)`` tuples or
                   ``np.ndarray``.
