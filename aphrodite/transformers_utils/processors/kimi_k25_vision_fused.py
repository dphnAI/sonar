...
from aphrodite.utils.jit_monitor import numba_workqueue_threading_layer
...
if is_numba_available():
    ...
    def _write_fused_patches(
        ...
        for linear in prange(total):
            ...
            patch_idx = out_offset + t * patch_h * patch_w + (y // patch_size) * patch_w + (x // patch_size)
...
class KimiK25FusedVisionProcessor(BaseImageProcessor):
    ...
    def __init__(self, media_proc_cfg: dict[str, Any], **kwargs: Any) -> None:
        ...
        self.normalize_lut = (values / 255.0 - image_mean[None, :]) * image_std_inv[None, :]
    ...
    def preprocess(
        ...
        for idx, item in enumerate(medias):
            ...
            if item["type"] == "image":
                ...
                frames = np.stack(
                    [self.resize_image(frame, new_width, new_height) for frame in item["video_chunk"]],
                    axis=0,
                )
            else:
        ...
        pixel_values_np = np.empty((total_patches, 3, patch_size, patch_size), dtype=np.float32)
        out_offset = 0
