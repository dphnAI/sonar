...
def _normalize_mrope_parameters(config: NemotronHConfig) -> None:
    """Normalize the checkpoint's mRoPE configuration for Aphrodite."""
...
class Cosmos3EdgeConfig(PretrainedConfig):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.text_config = Cosmos3EdgeTextConfig(**text_config) if isinstance(text_config, dict) else text_config
        _normalize_mrope_parameters(self.text_config)

        self.vision_config = (
            Cosmos3EdgeVisionConfig(**vision_config) if isinstance(vision_config, dict) else vision_config
        )
        self.projector_config = (
            Cosmos3EdgeProjectorConfig(**projector_config) if isinstance(projector_config, dict) else projector_config
        )
