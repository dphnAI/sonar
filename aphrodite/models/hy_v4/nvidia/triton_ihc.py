...
def _ihc_pre_stage2(
    ...
    if hidden_block_idx == 0:
        post = MAGNITUDE * tl.sigmoid(post_mix * reciprocal_rms * post_scale + post_base) + HC_EPS
        tl.store(
...
def triton_ihc_post(
    ...
    output = torch.empty((num_tokens, hc_mult, hidden_size), dtype=x.dtype, device=x.device)
