...
``index_topk``). Adapted to Aphrodite's paged KV cache: the KV page size is forced to
equal the sparse block size (128), so one selected block maps to exactly one
page.

Main K/V cache layout (Aphrodite):
...
def _kv_scale_args(
    ...
    if k_scale.dim() == 2 and v_scale.dim() == 2:
        if k_scale.shape[0] != num_kv_heads or v_scale.shape[0] != num_kv_heads:
            raise ValueError(f"per-token/head KV scales must have shape [{num_kv_heads}, max_kv_tokens]")
    ...
    raise ValueError(
        "MiniMax-M3 sparse attention supports scalar KV scales or [num_kv_heads, max_kv_tokens] per-token/head scales"
    )
