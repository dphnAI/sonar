...
from aphrodite.platforms.rocm import on_gfx950, on_mi3xx
    ...
    for j in range(real_q_loop):
        ...
        for _ in tl.range(real_topk):
            ...
            for sub_i in range(NUM_SUB):
                ...
                k_sub = tl.load(
                    kv_base + off_sub[None, :] * stride_kv_pos + off_d[:, None] * stride_kv_d,
                ...
                if USE_FP8:
                    ...
                    if KV_SCALE_MODE == 1:
                        ...
                        k_scale = tl.load(
                            k_scale_ptr + pid_kh * stride_ks_h + (page * BLOCK_SIZE_K + off_sub) * stride_ks_t,
                            mask=pos_mask_sub,
                            other=1.0,
                        )
                ...
                v_sub = tl.load(
                    kv_base + off_sub[:, None] * stride_kv_pos + (head_dim + off_d[None, :]) * stride_kv_d,
                ...
                if USE_FP8:
                    ...
                    if KV_SCALE_MODE == 1:
                        ...
                        v_scale = tl.load(
                            v_scale_ptr + pid_kh * stride_vs_h + (page * BLOCK_SIZE_K + off_sub) * stride_vs_t,
                            mask=pos_mask_sub,
                            other=1.0,
                        )
