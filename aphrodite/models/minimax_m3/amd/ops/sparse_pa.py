...
def minimax_m3_build_sparse_block_table_prefill(
    topk_idx: torch.Tensor,
    block_table: torch.Tensor,
    query_req_id: torch.Tensor,
    query_abs_pos: torch.Tensor,
) -> tuple[torch.Tensor, torch.Tensor]:
...
def minimax_m3_build_sparse_block_table_decode(
    ...
    query_abs_pos = (seq_lens[query_req_id] - decode_query_len + q_offset).to(torch.int32)
    return minimax_m3_build_sparse_block_table_prefill(topk_idx, block_table, query_req_id, query_abs_pos)
...
def _run_gluon_decode(
    ...
    if is_fp8:
        ...
        v_scale_arg = _gluon_scale_arg(v_scale, num_phys_pages=nphys16, num_kv_heads=hkv)
    else:
