...
def _get_unk_token_id(tokenizer, role: str) -> int:
    ...
    if eos is not None:
        logger.warning(
            "VocabMapping: %s has no unk_token_id; falling back to eos_token_id=%d for out-of-intersection tokens",
            role,
            eos,
        )
        return eos
    raise ValueError(
        f"VocabMapping: {role} has neither unk_token_id nor eos_token_id; cannot safely map out-of-intersection tokens"
    )
