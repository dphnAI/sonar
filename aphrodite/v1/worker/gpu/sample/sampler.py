...
class Sampler:
    ...
    def apply_sampling_params(
        ...
        # The ops below upcast to fp32 internally, so the input dtype is kept
        # and mutated in place. Only raw_logprobs reads the unmodified logits
        # afterward, so copy just for that case.
        if self.logprobs_mode.startswith("raw_") and self.returns_logprobs(idx_mapping_np):
            logits = logits.clone()
    ...
    def sample(
        ...
        use_flashinfer = self.use_flashinfer and not (
            ...
            or (return_logprobs and self.logprobs_mode in ("processed_logprobs", "processed_logits"))
