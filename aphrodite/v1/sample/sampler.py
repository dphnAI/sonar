...
class Sampler(nn.Module):
    ...
    def _execute_samplers_in_order(
        ...
        if not temperature_last_flags:
            return self._apply_sampler_order(
                logits, sampling_metadata, do_temperature_last=False
            )

        if all(temperature_last_flags):
            return self._apply_sampler_order(
                logits, sampling_metadata, do_temperature_last=True
            )
        ...
        temp_last_indices = [
            i for i, enabled in enumerate(temperature_last_flags) if enabled
        ]
        default_indices = [
            i for i, enabled in enumerate(temperature_last_flags) if not enabled
        ]

        if default_indices:
            default_metadata = self._subset_sampling_metadata(
                sampling_metadata, default_indices
            )
            default_logits = self._apply_sampler_order(
                logits[default_indices], default_metadata, False
            )
            logits[default_indices] = default_logits

        if temp_last_indices:
            temp_last_metadata = self._subset_sampling_metadata(
                sampling_metadata, temp_last_indices
            )
            temp_last_logits = self._apply_sampler_order(
                logits[temp_last_indices], temp_last_metadata, True
            )
    ...
    def _apply_sampler_order(
        ...
        for sampler_id in sampler_order:
            if (
                sampler_id == SamplerID.DRY
                and sampling_metadata.dry_multiplier is not None
            ):
                logger.debug(
                ...
                sampler_id == SamplerID.PENALTIES
                ...
                sampler_id == SamplerID.NO_REPEAT_NGRAM
                and sampling_metadata.no_repeat_ngram_size is not None
            ):
                logger.debug(
                ...
                sampler_id == SamplerID.TEMPERATURE
                and sampling_metadata.temperature is not None
            ):
                logger.debug(
                    "Applying temperature: %s", sampling_metadata.temperature
                )
                logits = self.apply_temperature(logits, sampling_metadata)

            elif (
                sampler_id == SamplerID.TOP_NSIGMA
                and sampling_metadata.top_nsigma is not None
            ):
                logger.debug(
                ...
                sampler_id == SamplerID.TOP_A
                and sampling_metadata.top_a is not None
            ):
                ...
                sampler_id == SamplerID.ETA_CUTOFF
                and sampling_metadata.eta_cutoff is not None
            ):
                logger.debug(
                ...
                sampler_id == SamplerID.EPSILON_CUTOFF
                and sampling_metadata.epsilon_cutoff is not None
            ):
                logger.debug(
                ...
                sampler_id == SamplerID.TYPICAL_P
                and sampling_metadata.typical_p is not None
            ):
                logger.debug(
                ...
                sampler_id == SamplerID.QUADRATIC
                and sampling_metadata.quadratic_smoothing_factor is not None
            ):
                logger.debug(
                ...
                sampler_id == SamplerID.XTC
                and sampling_metadata.xtc_threshold is not None
            ):
                logger.debug(
    ...
    def _subset_sampling_metadata(
        ...
        return replace(
            ...
            dry_allowed_length=maybe_index_tensor(
                sampling_metadata.dry_allowed_length
            ),
            dry_sequence_breaker_ids=maybe_index_tensor(
            ...
            dry_max_occurrences=maybe_index_tensor(
                sampling_metadata.dry_max_occurrences
            ),
            dry_early_exit_match_len=maybe_index_tensor(
                sampling_metadata.dry_early_exit_match_len
            ),
            no_repeat_ngram_size=maybe_index_tensor(
                sampling_metadata.no_repeat_ngram_size
            ),
            ...
            quadratic_smoothing_factor=maybe_index_tensor(
                sampling_metadata.quadratic_smoothing_factor
            ),
            quadratic_smoothing_curve=maybe_index_tensor(
                sampling_metadata.quadratic_smoothing_curve
            ),
            ...
            frequency_penalties=maybe_index_tensor(
                sampling_metadata.frequency_penalties
            ),
            presence_penalties=maybe_index_tensor(
                sampling_metadata.presence_penalties
            ),
            repetition_penalties=maybe_index_tensor(
                sampling_metadata.repetition_penalties
            ),
            output_token_ids=[sampling_metadata.output_token_ids[i] for i in indices],
            allowed_token_ids_mask=maybe_index_tensor(
                sampling_metadata.allowed_token_ids_mask
            ),
            bad_words_token_ids=reindex_dict_nested_list(
            ...
            output_token_ids_tensor=maybe_index_tensor(
                sampling_metadata.output_token_ids_tensor
            ),
            token_history_ids=maybe_index_tensor(
                sampling_metadata.token_history_ids
            ),
            token_history_lens=maybe_index_tensor(
                sampling_metadata.token_history_lens
            ),
            token_history_ids_cpu=maybe_index_tensor(
                sampling_metadata.token_history_ids_cpu
            ),
            token_history_lens_cpu=maybe_index_tensor(
                sampling_metadata.token_history_lens_cpu
            ),
            dry_multiplier_cpu=maybe_index_tensor(
                sampling_metadata.dry_multiplier_cpu
            ),
            dry_allowed_length_cpu=maybe_index_tensor(
                sampling_metadata.dry_allowed_length_cpu
            ),
            dry_sequence_breaker_ids_cpu=maybe_index_tensor(
                sampling_metadata.dry_sequence_breaker_ids_cpu
            ),
            dry_ranges_cpu=maybe_index_tensor(sampling_metadata.dry_ranges_cpu),
            dry_max_ngram_cpu=maybe_index_tensor(
                sampling_metadata.dry_max_ngram_cpu
            ),
            dry_max_occurrences_cpu=maybe_index_tensor(
                sampling_metadata.dry_max_occurrences_cpu
            ),
            dry_early_exit_match_len_cpu=maybe_index_tensor(
