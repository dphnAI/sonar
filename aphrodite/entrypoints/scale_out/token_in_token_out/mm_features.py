    ...
    *,
    ...
    for item in items:
        ...
        metadata_item = MultiModalKwargsItem(
            {key: elem for key, elem in item.items() if elem.field.keep_on_cpu or key in declared}
        )
        metadata_items.append(encode_mm_kwargs_item(metadata_item) if metadata_item else None)
...
def _encode_mm_kwargs_with_metadata(
    ...
    *,
    ...
    for modality, items in raw_mm_kwargs.items():
        ...
        declared = set(metadata_fields_for(modality)) if metadata_fields_for is not None else set()
    ...
    mm_metadata = (
        ...
        if any(item is not None for items in metadata_by_modality.values() for item in items)
        else None
    )
...
def mm_kwargs_from_features(
    ...
    for modality, hashes in features.mm_hashes.items():
        n = len(hashes)
        kwargs_items = [
            decode_mm_kwargs_item(item) if item is not None else None for item in kwargs_data.get(modality, [None] * n)
        ]
        metadata_items = [
            decode_mm_kwargs_item(item) if item is not None else None for item in mm_metadata.get(modality, [None] * n)
        ]
        mm_kwargs[modality] = [
            merge_mm_kwargs_items(kwargs_item, metadata_item)
            for kwargs_item, metadata_item in zip(kwargs_items, metadata_items, strict=True)
        ]
    return mm_kwargs
