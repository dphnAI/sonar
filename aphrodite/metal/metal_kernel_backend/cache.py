...
class MetalPagedKVCache:
        ...
        *,
        ...
        if turboquant:
            if k_quant is None or k_quant not in QUANT_PARAMS:
                ...
                raise ValueError(f"turboquant requires valid k_quant, got {k_quant!r}. Available: {available}")
            ...
            if v_quant not in V_QUANT_PARAMS:
                ...
                raise ValueError(f"turboquant requires valid v_quant, got {v_quant!r}. Available: {available}")
            if head_dim % 32 != 0:
                raise ValueError(f"TurboQuant requires head_dim divisible by 32, got {head_dim}")
            if head_dim not in FWHT_SUPPORTED_HEAD_DIMS:
                supported_head_dims = ", ".join(str(dim) for dim in FWHT_SUPPORTED_HEAD_DIMS)
                raise ValueError(f"TurboQuant V FWHT only supports head_dim in ({supported_head_dims}), got {head_dim}")

        self.k_size = QUANT_PARAMS[k_quant]["dtype"] if (turboquant and k_quant is not None) else None

        # Bit-packed dimensions for sub-8-bit quant types
        if turboquant:
            assert k_quant is not None and v_quant is not None
        ...
        if len(self.kv_heads_per_layer) != num_layers:
            raise ValueError(f"kv_heads_per_layer length {len(self.kv_heads_per_layer)} != num_layers {num_layers}")
        if len(self.head_dim_per_layer) != num_layers:
            raise ValueError(f"head_dim_per_layer length {len(self.head_dim_per_layer)} != num_layers {num_layers}")
        if len(self.sliding_window_per_layer) != num_layers:
            raise ValueError(
                f"sliding_window_per_layer length {len(self.sliding_window_per_layer)} != num_layers {num_layers}"
            )
        ...
        if not turboquant:
            ...
            kv_bytes = num_layers * num_blocks * block_size * num_kv_heads * head_dim * 2 * self._dtype_size(dtype)
            logger.info(
                "KV cache: %.1f MB (%d layers, %d blocks, %d tokens/block)",
                kv_bytes / 1e6,
                num_layers,
                num_blocks,
                block_size,
            ...
            fp16_equivalent = num_layers * num_blocks * block_size * num_kv_heads * head_dim * 2 * 2  # fp16 K+V
            compression = fp16_equivalent / tq_total if tq_total > 0 else float("inf")
            logger.info(
                "TurboQuant KV cache (packed): %.1f MB (K: %db->%dd, V: %db->%dd, vs %.1f MB fp16, %.2fx compression)",
                tq_total / 1e6,
                self.k_bits,
                self.k_packed_dim,
                self.v_bits,
    ...
    def _dtype_size(dtype: mx.Dtype) -> int:
        ...
        if size is None:
            raise ValueError(
                f"Unknown dtype {dtype} in _dtype_size. Supported: {list(MetalPagedKVCache._DTYPE_SIZES.keys())}"
            )
        return size
