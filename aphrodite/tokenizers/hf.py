...
def maybe_make_thread_pool(tokenizer: _T, copies: int = 1):
    ...
    """
    if not isinstance(tokenizer, TokenizersBackend) or isinstance(tokenizer, ThreadSafeHFTokenizerMixin):
        return tokenizer
...
def get_cached_tokenizer(tokenizer: HfTokenizer) -> HfTokenizer:
    ...
    if getattr(getattr(tokenizer, "tokenizer", None), "instruct_tokenizer", None):
        from aphrodite.tokenizers.mistral import mistral_common_tekkenizer
    ...
    class CachedTokenizer(tokenizer.__class__):  # type: ignore
        ...
        def convert_ids_to_tokens(self, ids, skip_special_tokens: bool = False):
            if mistral_tekkenizer is not None:
                from aphrodite.tokenizers.mistral import tekken_convert_ids_to_tokens
        ...
        def convert_tokens_to_string(self, tokens: list[str]) -> str:
            if mistral_tekkenizer is not None:
                from aphrodite.tokenizers.mistral import tekken_convert_tokens_to_string
