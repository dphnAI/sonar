...
class XPUFp8BlockScaledMMKernel(Fp8BlockScaledMMLinearKernel):
    ...
    def process_weights_after_loading(self, layer: torch.nn.Module) -> None:
        ...
        if getattr(layer, "is_bmm", False):
            ...
            layer.bmm_scale = scale_t.reshape(k_blocks, batch, n_per_batch_blocks).permute(1, 0, 2).contiguous()
            # Precompute [G, K, N] weight for fp8_bmm.
            # Original weight is [N_total, K] where N_total = G * N_per_group.
            w = layer.weight.data
            N_total, K = w.shape
            N_per_group = N_total // batch
            layer.bmm_weight = w.reshape(batch, N_per_group, K).permute(0, 2, 1)
