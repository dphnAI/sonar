...
def _on_gfx12x() -> bool:
    if not current_platform.is_rocm():
        return False
    from aphrodite.platforms.rocm import on_gfx12x
...
def _on_gfx1151() -> bool:
    if not current_platform.is_rocm():
        return False
    from aphrodite.platforms.rocm import on_gfx1151
...
def triton_w4a16_skinny_fmt_gemm(
    ...
    assert scales.shape == (N, num_groups), f"scales shape mismatch: {scales.shape} vs ({N}, {num_groups})"
    if zp is not None:
        assert zp.is_contiguous(), "Zero-points must be contiguous"
        assert zp.shape == (N, num_groups), f"zp shape mismatch: {zp.shape} vs ({N}, {num_groups})"
    ...
    if _on_gfx12x():
        ...
        override = _GFX1X_PREFILL_OVERRIDES.get((group_size, K, N)) if M <= 128 else None
...
class RDNAHybridW4A16LinearKernel(MPLinearKernel):
    ...
    def process_weights_after_loading(self, layer: torch.nn.Module) -> None:
        ...
        unpacked = unpack_quantized_values_into_int32(w_q_raw.data, c.weight_type, packed_dim=w_q_raw.packed_dim)
        ...
        if c.zero_points:
            ...
            zp_unpacked = unpack_quantized_values_into_int32(w_zp_raw.data, c.weight_type, packed_dim=0)
    ...
    def apply_weights(
        ...
        output = torch.ops.aphrodite.rdna_hybrid_w4a16_apply(
            x_2d,
            w_q,
            w_s,
            w_zp,
