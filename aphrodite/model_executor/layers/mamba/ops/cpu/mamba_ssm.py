...
def _mamba_chunk_scan_combined_fwd_cpu(
    ...
    **kwargs,
    ...
    all_states = torch.zeros(batch, nheads, headdim, dstate, dtype=torch.float32, device=x.device)
    if initial_states is not None:
        all_states.copy_(initial_states.float())

    assert out.is_contiguous(), "_mamba_chunk_scan_combined_fwd_cpu: `out` must be pre-allocated as a contiguous tensor"
...
def selective_state_update(
    ...
    _dt_bias = dt_bias.unsqueeze(0) if (dt_bias is not None and dt_bias.dim() == 1) else dt_bias
