...
class CompressedTensorsWNA8Int(CompressedTensorsScheme):
    ...
    def __init__(
        ...
        if num_bits not in WNA16_SUPPORTED_TYPES_MAP:
            raise ValueError(
                f"Unsupported num_bits = {num_bits} for {self._scheme_name}; "
        ...
        if input_quant is not None:
            if not input_quant.symmetric:
                raise ValueError(
                    f"{self._scheme_name} requires symmetric activation quantization, "
                    f"got symmetric={input_quant.symmetric}"
                )
            if not input_quant.dynamic:
                raise ValueError(
                    f"{self._scheme_name} requires dynamic activation quantization, got dynamic={input_quant.dynamic}"
                )
            if input_quant.strategy not in (
                ...
                raise ValueError(
                    f"{self._scheme_name} requires per-token or per-group activation "
    ...
    def _build_input_quant_config(self) -> dict | None:
        ...
        strategy_val = iq.strategy.value if hasattr(iq.strategy, "value") else iq.strategy
        return {
    ...
    def create_weights(
        ...
        **kwargs,
        ...
        if kernel_type.__name__ not in self._kernel_backends_being_used:
            logger.info(
                "Using %s for CompressedTensors%s",
        ...
        partition_scales = not marlin_repeat_scales_on_all_ranks(False, self.group_size, row_parallel)
        ...
        scale_data = torch.empty(output_size_per_partition, scales_size, dtype=params_dtype)
        if partition_scales:
