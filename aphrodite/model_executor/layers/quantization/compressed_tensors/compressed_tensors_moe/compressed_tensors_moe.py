...
class CompressedTensorsMoEMethod(FusedMoEMethodBase):
    ...
    def get_moe_method(
        ...
        if quant_config._is_wNa16_group_channel(weight_quant, input_quant):
            ...
            is_actorder = weight_quant.strategy == QuantizationStrategy.GROUP and weight_quant.actorder in (
                ActivationOrdering.GROUP,
                ActivationOrdering.DYNAMIC,
            )
            if (
                not check_moe_marlin_supports_layer(layer, group_size, allow_tile_padding=not is_actorder)
                ...
                if current_platform.is_rocm():
                    ...
                    if (
                        ...
                        # This class also routes explicit emulation requests
                        # through select_wna16_moe_backend.
                        from .compressed_tensors_moe_wna16_marlin import (
                            CompressedTensorsWNA16MarlinMoEMethod,
                        )

                        logger.info_once("Using CompressedTensorsWNA16MarlinMoEMethod (emulation backend requested)")
