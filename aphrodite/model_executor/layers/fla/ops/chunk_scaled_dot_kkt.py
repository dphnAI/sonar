...
# On RDNA (gfx11xx/gfx12xx) WMMA only accepts 16-bit/int inputs, so a
# widened (e.g. fp32) tl.dot is lowered to a software matmul (~190x
# amdgcn-stage blowup). There we cast both operands down to k's native
# storage dtype (bf16/fp16) so fast WMMA is used instead.
...
def chunk_scaled_dot_kkt_fwd_kernel(
    ...
    for i_k in range(tl.cdiv(K, BK)):
        ...
        if CAST_DOT_TO_K_DTYPE:
            ...
            # Keep the promoted precision of the beta-scaled operand.
