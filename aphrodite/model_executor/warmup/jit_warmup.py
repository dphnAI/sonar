...
def _expand_warmup_value_grid(
    ...
    return tuple(dict(zip(names, value_set)) for value_set in itertools.product(*expanded_values))
...
class _CompileKeyDispatchTrace:
    ...
    def compile_key(
        ...
        for name, expr in self.local_exprs:
            dispatch_values[name] = _eval_dispatch_expr(expr, dispatch_values, self.globals)
        return compile_key_type(
            **{field: _eval_dispatch_expr(expr, dispatch_values, self.globals) for field, expr in self.field_exprs}
        )
    ...
    for statement in function_def.body:
        ...
        if isinstance(statement, ast.Assign):
            if len(statement.targets) != 1 or not isinstance(statement.targets[0], ast.Name):
                raise _dispatch_expr_error(statement, "Dispatch assignments must target one local name")
            local_exprs.append((statement.targets[0].id, statement.value))
            continue

        if isinstance(statement, ast.AnnAssign):
            if statement.value is None:
                raise _dispatch_expr_error(statement, "Dispatch annotations must assign a value")
        ...
        if isinstance(statement, ast.Return):
            raise _dispatch_expr_error(statement, "Dispatch must return one CompileKey(...) call")
        ...
        def traced(
            ...
            **kwargs: WarmupValues,
            ...
            expanded_input_groups = tuple(
                _expand_warmup_input_rows(group.rows, compile_key_dispatch_trace.input_names) for group in input_groups
            )
            expanded_kwargs = _expand_warmup_value_grid(kwargs, compile_key_dispatch_trace.input_names)
            dispatch_value_groups = (*expanded_input_groups, expanded_kwargs)
            return list(
                dict.fromkeys(
                    compile_key_dispatch_trace.compile_key(self.CompileKey, _merge_warmup_kwargs(dispatch_values))
