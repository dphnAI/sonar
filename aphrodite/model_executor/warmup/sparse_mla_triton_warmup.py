...
def _compile_sparse_swa_prefill_metadata_kernel(
    ...
    _COMPUTE_PREFILL_METADATA_KERNEL.warmup(aphrodite_config)
...
def _compile_prefill_chunk_metadata_kernel(
    ...
    _BUILD_PREFILL_CHUNK_METADATA_KERNEL.warmup(aphrodite_config)
...
def _compile_combine_topk_swa_indices_kernel(
    ...
    _COMBINE_TOPK_SWA_INDICES_KERNEL.warmup(aphrodite_config)
...
def sparse_mla_triton_warmup(worker: "Worker") -> None:
    ...
    try:
        if _has_attention_backend(runner, _DEEPSEEK_V4_SPARSE_MLA_BACKENDS):
            _compile_sparse_swa_prefill_metadata_kernel(aphrodite_config)
            _compile_prefill_chunk_metadata_kernel(aphrodite_config)
            _compile_combine_topk_swa_indices_kernel(aphrodite_config)
