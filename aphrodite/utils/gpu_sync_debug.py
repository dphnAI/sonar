...
def enable_gpu_sync_check() -> None:
    ...
    first-compile is complete. No-op unless `APHRODITE_GPU_SYNC_CHECK` is set."""
    if envs.APHRODITE_GPU_SYNC_CHECK is None:
        return
...
if current_platform.is_cuda_alike():
    ...
    def gpu_sync_allowed(first_only: bool = False):
        ...
        """
        if envs.APHRODITE_GPU_SYNC_CHECK is None or torch.compiler.is_compiling():
            return _noop_cm()
        prev_mode = torch.cuda.get_sync_debug_mode()
        if not prev_mode:
            return _noop_cm()
