...
def release_device_memory_under_pressure(device: torch.device) -> bool:
    ...
    releasable = torch.accelerator.memory_reserved(device) - torch.accelerator.memory_allocated(device)
...
def memory_profiling(
    ...
    result.total_consumed = result.before_create.free_memory - result.after_profile.free_memory

    # total_consumed already covers persistent torch allocations; add only the
    # transient peak headroom to avoid double-counting.
    result.transient_peak_headroom = result.after_profile.torch_peak - result.after_profile.torch_allocated
