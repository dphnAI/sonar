...
def do_shrink_kernel_fp8(
    ...
    if SLICE_NUM == 1:
        ...
        cur_lora_ptr = (
            ...
            else tl.load(lora_ptr + slice_id).to(tl.pointer_type(input_ptr.dtype.element_ty))
        )
        cur_b_scale_ptr = (
            tl.load(b_scale_ptr + slice_id).to(tl.pointer_type(tl.float32)) if b_scale_ptr is not None else b_scale_ptr
        )
    ...
    if use_fp8_w8a8:
        if group_k > 0 and group_n > 0:
            ...
            mm_b_scale_ptr = cur_b_scale_ptr + lora_index * b_scale_l_stride + offs_ns * b_scale_n_stride
        elif per_channel_quant:
            # Per-channel for weights, per-token for activations
            b_scale_ptrs = cur_b_scale_ptr + lora_index * b_scale_l_stride + rbn * b_scale_n_stride
...
def do_expand_kernel_fp8(
    ...
    if SLICE_NUM == 1:
        ...
        if use_fp8_w8a8:
            cur_lora_ptr = tl.load(lora_ptr + slice_id).to(tl.pointer_type(tl.float8e4nv))
            cur_b_scale_ptr = tl.load(b_scale_ptr + slice_id).to(tl.pointer_type(tl.float32))
        else:
            cur_lora_ptr = tl.load(lora_ptr + slice_id).to(tl.pointer_type(out_ptr.dtype.element_ty))
            cur_b_scale_ptr = (
                tl.load(b_scale_ptr + slice_id).to(tl.pointer_type(tl.float32)) if b_scale_ptr is not None else None
            )
