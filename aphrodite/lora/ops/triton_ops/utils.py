...
def _get_lora_b_ptr(lora_weights: list[torch.Tensor], offset_start: int, device: torch.device):
    ...
    if len(lora_weights) > 1:
        lora_ptr_tensor = async_tensor_h2d(tensor_ptrs, dtype=torch.uint64, device=device)
        slice_start_tensor = async_tensor_h2d(slice_offset_lst, dtype=torch.uint64, device=device)
    else:
    ...
    if (len(set(lora_strides_d0)) == 1 and len(set(lora_strides_d1)) == 1 and len(set(lora_strides_d2)) == 1) and len(
        ...
        lora_strides_d0_tensor = async_tensor_h2d(lora_strides_d0, dtype=torch.int64, device=device)
        lora_strides_d1_tensor = async_tensor_h2d(lora_strides_d1, dtype=torch.int64, device=device)
        lora_strides_d2_tensor = async_tensor_h2d(lora_strides_d2, dtype=torch.int64, device=device)
        hidden_sizes_tensor = async_tensor_h2d(hidden_sizes, dtype=torch.int64, device=device)
        same_stride = False
