...
fn serve_args_reject_unsupported_flag_arg() {
    let error = Cli::try_parse_from(["aphrodite-rs", "serve", "Qwen/Qwen3-0.6B", "--allow-credentials"])
        .unwrap_err();
...
fn serve_args_keep_frontend_arg_after_separator() {
    ...
    assert_eq!(args.python_args, vec!["--uds", "/tmp/aphrodite.sock"]);
}
...
fn serve_args_auto_forward_negative_value_without_separator() {
    let cli =
        Cli::try_parse_from(["aphrodite-rs", "serve", "Qwen/Qwen3-0.6B", "--dtype", "-1"]).unwrap();
