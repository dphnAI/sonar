...
/// Marker type for frontend-owned `serve` arguments that `aphrodite-rs` recognizes but does not
/// support yet.
///
...
/// Frontend-owned Python `serve` arguments that `aphrodite-rs` recognizes but does not support yet.
...
/// Frontend-owned Python `aphrodite serve` top-level arguments that `aphrodite-rs` recognizes but does not
/// support yet.
///
...
pub struct TopLevelUnsupportedArgs {
    ...
    /// pip install aphrodite[grpc].
    #[arg(long, default_missing_value = "true", num_args = 0..=1)]
    pub grpc: Option<Unsupported>,
}

/// Frontend-owned Python engine arguments that `aphrodite-rs` recognizes but does not support yet.
///
/// Source of truth in Python Aphrodite:
...
/// Frontend-owned Python OpenAI server arguments that `aphrodite-rs` recognizes but does not support
/// yet.
///
/// Source of truth in Python Aphrodite:
