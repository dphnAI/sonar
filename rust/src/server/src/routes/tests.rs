...
impl Tokenizer for FakeChatTokenizer {
    fn encode(&self, text: &str, _add_special_tokens: bool) -> aphrodite_text::Result<Vec<u32>> {
        Ok(text.bytes().map(u32::from).collect())
    }

    fn decode(&self, token_ids: &[u32], _skip_special_tokens: bool) -> aphrodite_text::Result<String> {
        Ok(
...
impl Tokenizer for FailingDecodeTokenizer {
    fn encode(&self, text: &str, add_special_tokens: bool) -> aphrodite_text::Result<Vec<u32>> {
        FakeChatTokenizer.encode(text, add_special_tokens)
    }

    fn decode(&self, token_ids: &[u32], skip_special_tokens: bool) -> aphrodite_text::Result<String> {
        if token_ids.contains(&(b'i' as u32)) {
            return Err(aphrodite_text::Error::Tokenizer(
