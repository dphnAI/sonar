...
use aphrodite_engine_core_client::protocol::{Logprobs, PositionLogprobs};
use aphrodite_llm::{CollectedGenerateOutput, GenerateOutputStreamExt as _};
...
/// Validate one token-in/token-out request and proxy it into the shared `aphrodite-text` stack.
