...
pub struct PreparedRequest {
    ...
    /// Lowered text-only chat request for `aphrodite-chat`.
...
pub(crate) fn prepare_chat_request(
    ...
    // Auto-enable prompt logprobs for non-streaming echo, matching Python Aphrodite's behavior.
...
fn convert_message(message: ChatMessage) -> Result<AphroditeChatMessage, ApiError> {
    match message {
...
/// Convert the given OpenAI message content value into the internal format in `aphrodite-chat`.
...
/// Convert the given OpenAI assistant message content into the internal format in `aphrodite-chat`.
...
mod tests {
    ...
    use serde_json::json;
    use aphrodite_chat::{
        AssistantContentBlock, AssistantToolCall, ChatMessage as AphroditeChatMessage,
