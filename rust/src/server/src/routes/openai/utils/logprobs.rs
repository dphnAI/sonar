...
///
/// The first prompt token is included with `None` logprob metadata, matching Python Aphrodite's
...
/// Convert decoded prompt logprobs into the Aphrodite-style prompt-logprobs response shape.
...
///
/// When `echoed_prompt` is true, the returned payload matches Python Aphrodite's echoed completions
