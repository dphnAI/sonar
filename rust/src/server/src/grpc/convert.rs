//! Conversion between gRPC protobuf types and internal `aphrodite-text` request/response types.
...
pub fn to_text_request(
    ...
    if let Some(kv) = kv {
        // Thread kv_transfer_params through aphrodite_xargs, matching the HTTP route convention.
