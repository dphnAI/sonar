...
pub fn lower_sampling_params(
    ...
    // Mirrors the model-generation-config inheritance used by Aphrodite's OpenAI chat path.
    // If neither the caller nor the model provides a value, fall back to 1.0 — the default
    // used by the Python Aphrodite OpenAI-compatible API (via `_DEFAULT_SAMPLING_PARAMS`).
...
/// Convert bad-word strings into token-ID sequences, following the Python Aphrodite logic in
...
/// Resolve the effective `max_tokens` for generation, mirroring Aphrodite Python's `get_max_tokens()`
/// in `aphrodite/entrypoints/utils.py`.
///
