...
Run the Rust smoke test through the `aphrodite-engine-core-client` utility interface:

```bash
cargo run -p aphrodite-engine-core-client --example external_engine_utility_call -- \
...
```bash
cargo run -p aphrodite-engine-core-client --example external_engine_logprobs -- \
  --handshake-address tcp://127.0.0.1:62100 \
  --host 127.0.0.1
```
