...
def prepare_hybrid_weights(K, N, group_size, device="cuda"):
    ...
    w_q_skinny_i32 = torch.randint(0, 2**31, (N, K // 8), dtype=torch.int32, device=device)
    ...
    w_zp = torch.randint(0, 16, (N, num_groups), dtype=torch.int32, device=device).to(torch.float16)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Benchmark RDNAHybridW4A16LinearKernel")
    parser.add_argument(
        "--models",
        nargs="+",
        type=str,
