...
import aphrodite
from aphrodite.models.hy_v4.nvidia.triton_ihc import (
    triton_ihc_post,
    triton_ihc_pre,
)
...
def eager_post(
    ...
    return (post.float().unsqueeze(-1) * x.float().unsqueeze(-2) + residual.float()).to(x.dtype)
...
def run_benchmark(
    ...
    print(f"aphrodite: {aphrodite.__version__}; torch: {torch.__version__}; CUDA: {torch.version.cuda}")
    print(
        f"triton: {triton.__version__}; flashinfer: {version('flashinfer-python')}; "
        f"dtype: {dtype}; method: {method}; cache: cold L2"
    )
    print(
        "env: APHRODITE_ENABLE_HPC_OPS="
        f"{os.getenv('APHRODITE_ENABLE_HPC_OPS', '<unset>')}; "
        "APHRODITE_BATCH_INVARIANT="
        f"{os.getenv('APHRODITE_BATCH_INVARIANT', '<unset>')}"
    )
    ...
    for num_tokens in token_counts:
        ...
        triton_output, triton_post = triton_ihc_pre(x, weight, scale, base, 2.0, 1e-6, 1e-5)
        ...
        benchmarks = (
            (
                ...
                x.nbytes + weight.nbytes + scale.nbytes + base.nbytes + eager_output.nbytes + post.nbytes,
            ),
            (
                "post",
