...
def _create_malicious_sparse_tensor() -> torch.Tensor:
    ...
    shape = (3, 3)
    return torch.sparse_coo_tensor(indices, values, shape, dtype=torch.float32, check_invariants=False)
...
class TestConcurrentRaceProtection:
    ...
    def test_forced_interleaving_still_rejects_invalid(self, model_config):
        ...
        def thread_b_malicious():
            try:
                ...
                with check_sparse_tensor_invariants_threadsafe():
                    tensor = torch.load(
                        io.BytesIO(pybase64.b64decode(malicious_encoded, validate=True)),
                        weights_only=True,
                        map_location=torch.device("cpu"),
                    )
                    tensor.to_dense()
        ...
        assert results.get("b_rejected") is True, "Malicious tensor was NOT rejected under concurrency"
    ...
    def test_concurrent_valid_loads_succeed(self, model_config):
        """Concurrent valid loads must all succeed."""
        valid_encoded = _encode_tensor(_create_valid_dense_tensor())
        num_workers = 4
        successes = []
        failures = []
...
class TestCrossLoaderLockSharing:
    ...
    def test_all_loaders_use_same_context_manager(self):
        ...
        import aphrodite.multimodal.media.audio as audio_mod
        import aphrodite.multimodal.media.image as image_mod
        import aphrodite.renderers.embed_utils as embed_mod
        from aphrodite.utils.sparse_utils import check_sparse_tensor_invariants_threadsafe as cm_from_utils
