...
from aphrodite.renderers.inkling import (
    InklingRenderer,
    _HfBackedTmlTokenizer,
    _resolve_reasoning_effort,
)
from aphrodite.renderers.inkling_encoding import (
...
def decode(token_ids):
    return "".join(_ID_TO_SPECIAL.get(tid, chr(tid) if tid < 200000 else f"<{tid}>") for tid in token_ids)
...
class TestRustFixtureParity:
    def test_tool_round_trip(self, inkling_tokenizer):
        ...
        assert render_text(inkling_tokenizer, messages, add_generation_prompt=False) == (
...
class TestRenderingSemantics:
    ...
    def test_tool_call_non_object_args_rejected(self, inkling_tokenizer):
        ...
        with pytest.raises(TypeError, match="decode to an object"):
            render_inkling_messages(messages, inkling_tokenizer, add_generation_prompt=False)
    ...
    def test_unsupported_role_rejected(self, inkling_tokenizer):
        with pytest.raises(ValueError, match="unsupported Inkling message role"):
            render_inkling_messages([{"role": "narrator", "content": "hi"}], inkling_tokenizer)
...
class TestReasoningEffort:
    ...
    def test_zero_effort_has_one_canonical_spelling(self, inkling_tokenizer, value):
        ...
        assert text.startswith("<|message_system|><|content_text|>Thinking effort level: 0.0<|end_message|>")

    def test_emits_one_effort_after_initial_prefix(self, inkling_tokenizer):
        effort_block = "<|message_system|><|content_text|>Thinking effort level: 0.7<|end_message|>"
        text = render_text(
            inkling_tokenizer,
            [
                {"role": "system", "content": "rules"},
...
class TestSpecialTokenResolution:
    ...
    def test_semantic_spelling_preferred(self):
        ...
        tools = [{"type": "function", "function": {"name": "f"}}]
        ids = render_inkling_messages([{"role": "user", "content": "hi"}], inkling_tokenizer, tools=tools)
