...
# --------------------------------------------------------------------------- sparse MLA v2 (h8 tile / split-KV)
def _run_sparse_mla_v2(
    q: torch.Tensor,
    pool: torch.Tensor,
    indices: torch.Tensor,
    sm_scale: float,
    ...
    out = torch.empty(T, h, DS_NOPE, device="cuda", dtype=torch.bfloat16)
    lse = torch.empty(T, h, device="cuda", dtype=torch.float32)
    part_o = part_ml = None
    if num_splits > 1:
        # NaN-poisoned: a slice the kernels fail to write (e.g. an empty split)
        # would leak NaN through the combine and trip the asserts.
        part_o = torch.full((num_splits, T, h, DS_NOPE), float("nan"), device="cuda", dtype=torch.float32)
        part_ml = torch.full((num_splits, T, h, 2), float("nan"), device="cuda", dtype=torch.float32)
    ops.sm89_sparse_mla_fwd_v2(q, pool, indices, out, lse, part_o, part_ml, sm_scale, num_splits, h8_tile)
    return out, lse


@pytest.mark.parametrize("h8_tile", [False, True])
@pytest.mark.parametrize("num_splits", [1, 2, 8])
@pytest.mark.parametrize("num_heads", [4, 8, 16])
@pytest.mark.parametrize(("num_tokens", "topk"), [(1, 2048), (8, 2048), (8, 64)])
def test_sm89_sparse_mla_fwd_v2(h8_tile, num_splits, num_heads, num_tokens, topk):
    """The gated h8-tile / split-KV paths against the fp64 oracle and the
    validated single-pass kernel. (h8_tile=False, num_splits=1) must match
    sm89_sparse_mla_fwd bitwise; topk=64 with num_splits=8 exercises empty
    splits (only 2 key iterations exist)."""
    torch.manual_seed(num_heads * 10007 + num_splits * 131 + num_tokens + topk)
    sm_scale = 576**-0.5
    num_slots = topk + 512
    pool, kv_deq = _make_ds_mla_pool(num_slots)
    q = torch.randn(num_tokens, num_heads, 576, device="cuda", dtype=torch.bfloat16)
    ...
    out_base = torch.empty(num_tokens, num_heads, DS_NOPE, device="cuda", dtype=torch.bfloat16)
    lse_base = torch.empty(num_tokens, num_heads, device="cuda", dtype=torch.float32)
    ops.sm89_sparse_mla_fwd(q, pool, indices, out_base, lse_base, sm_scale)

    out, lse = _run_sparse_mla_v2(q, pool, indices, sm_scale, num_splits, h8_tile)
    out_f = out.float()
    assert not torch.isnan(out_f).any() and not torch.isnan(lse).any()

    if not h8_tile and num_splits == 1:
        assert torch.equal(out, out_base) and torch.equal(lse, lse_base), "default v2 config must be bitwise v1"

    # the same oracle bars as the single-pass test
    ref_out, ref_lse = _sparse_mla_ref(q, kv_deq, indices, sm_scale)
    valid = (indices >= 0).any(dim=-1)
    if (~valid).any():
        assert (out_f[~valid] == 0).all(), "all-(-1) rows must produce exact-0 output"
    ...
    # cross-check against the validated single-pass kernel: identical math, so
    # only summation-order / partial-merge rounding may differ
    assert torch.equal(torch.isneginf(lse), torch.isneginf(lse_base))
    assert (out_f - out_base.float()).abs().max().item() < 3e-2
    assert (lse[valid] - lse_base[valid]).abs().max().item() < 1e-3

    # run-to-run determinism (fixed reduction orders, no atomics)
    out2, lse2 = _run_sparse_mla_v2(q, pool, indices, sm_scale, num_splits, h8_tile)
    assert torch.equal(out, out2) and torch.equal(lse, lse2)


def test_sm89_sparse_mla_v2_cuda_graph():
    torch.manual_seed(11)
    num_tokens, num_heads, topk, num_slots, num_splits = 2, 8, 512, 1024, 8
    sm_scale = 576**-0.5
    pool, _ = _make_ds_mla_pool(num_slots)
    q = torch.randn(num_tokens, num_heads, 576, device="cuda", dtype=torch.bfloat16)
    indices = _make_sparse_indices(num_tokens, topk, num_slots)
    out = torch.zeros(num_tokens, num_heads, DS_NOPE, device="cuda", dtype=torch.bfloat16)
    lse = torch.zeros(num_tokens, num_heads, device="cuda", dtype=torch.float32)
    part_o = torch.zeros(num_splits, num_tokens, num_heads, DS_NOPE, device="cuda", dtype=torch.float32)
    part_ml = torch.zeros(num_splits, num_tokens, num_heads, 2, device="cuda", dtype=torch.float32)

    def run():
        ops.sm89_sparse_mla_fwd_v2(q, pool, indices, out, lse, part_o, part_ml, sm_scale, num_splits, True)

    def eager() -> tuple[torch.Tensor, torch.Tensor]:
        out_e = torch.zeros_like(out)
        lse_e = torch.zeros_like(lse)
        part_o_e = torch.zeros_like(part_o)
        part_ml_e = torch.zeros_like(part_ml)
        ops.sm89_sparse_mla_fwd_v2(q, pool, indices, out_e, lse_e, part_o_e, part_ml_e, sm_scale, num_splits, True)
        torch.cuda.synchronize()
        return out_e, lse_e
