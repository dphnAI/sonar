...
def _amx_fp8_available() -> bool:
    ...
    return torch.cpu._is_amx_tile_supported() and torch.ops._C.cpu_attn_has_isa("amx_fp8")
...
@pytest.mark.parametrize("kv_cache_dtype", ["fp8_e4m3", "fp8_e5m2"])
@pytest.mark.skipif(not _amx_fp8_available(), reason="requires AMX_FP8")
def test_amx_fp8_softmax_rescales_small_logit_increases(kv_cache_dtype: str) -> None:
    # Span multiple KV tiles without splitting the reduction across threads.
    # The second half raises logits by 2, below the BF16 rescale threshold of 8.
    head_size, block_size, token_num = 64, 64, 65536
    query = torch.ones((1, 1, head_size), dtype=torch.bfloat16)
    key = torch.zeros((token_num, 1, head_size), dtype=torch.bfloat16)
    key[token_num // 2 :].fill_(0.25)
    value = torch.full_like(key, -1)
    value[token_num // 2 :].fill_(1)
    key_cache = torch.empty((token_num // block_size, 1, block_size, head_size), dtype=torch.uint8)
    value_cache = torch.empty_like(key_cache)
    cpu_attn_reshape_and_cache(
        key=key,
        value=value,
    ...
    query_start_loc = torch.tensor([0, 1], dtype=torch.int32)
    seq_lens = torch.tensor([token_num], dtype=torch.int32)
    metadata = cpu_attn_get_scheduler_metadata(
        num_reqs=1,
        num_heads=1,
        num_kv_heads=1,
    ...
    cpu_attention_with_kv_cache(
        ...
        alibi_slopes=None,
        sliding_window=-1,
        block_table=torch.arange(token_num // block_size, dtype=torch.int32).unsqueeze(0),
        softcap=0,
        scheduler_metadata=metadata,
        s_aux=None,
        kv_cache_dtype=kv_cache_dtype,
    )
    torch.testing.assert_close(output, torch.full_like(output, math.tanh(1)), atol=0.05, rtol=0)
