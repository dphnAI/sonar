...
class TestGetFp8MinMax:
    ...
    @patch("aphrodite.model_executor.layers.quantization.utils.quant_utils.current_platform")
    ...
    def test_non_fnuz_platform_uses_finfo(self, mock_platform):
        ...
        assert fp8_max == finfo.max, f"Non-fnuz platform should use finfo.max={finfo.max}, got {fp8_max}"
        assert fp8_min == finfo.min, f"Non-fnuz platform should use finfo.min={finfo.min}, got {fp8_min}"
