...
from aphrodite.platforms.rocm import on_gfx1x  # noqa: E402

device = "cuda"

hybrid_module = importlib.import_module("aphrodite.model_executor.kernels.linear.mixed_precision.rdna_hybrid_w4a16")
...
def test_rdna_hybrid_w4a16_apply_matches_reference(dtype, group_size, has_zp, M):
    ...
    scales_nkg = (0.05 * torch.rand((N, K // group_size), device=device, dtype=torch.float32)).to(dtype)

    # Optional raw zero points [N, K//G] in act dtype.
    if has_zp:
        zp_nkg = torch.randint(0, 16, (N, K // group_size), device=device, dtype=torch.int32).to(dtype)
    else:
        zp_nkg = None

    from aphrodite.utils.platform_utils import num_compute_units

    out = torch.ops.aphrodite.rdna_hybrid_w4a16_apply(
        x_mk,
        w_q,
        scales_nkg,
        zp_nkg,
    ...
    ref = _rdna_hybrid_w4a16_reference(x_mk, w_int4_nk, scales_nkg, zp_nkg, group_size, bias=None)
...
def test_rdna_hybrid_w4a16_apply_with_bias(dtype, M):
    ...
    scales_nkg = (0.05 * torch.rand((N, K // G), device=device, dtype=torch.float32)).to(dtype)
...
def test_pack_int4_exllama_shuffle_layout():
    ...
    shifts = torch.tensor([0, 16, 4, 20, 8, 24, 12, 28], device=device, dtype=torch.int32)
...
def _pack_int4_along_n_for_zp(zp_int4_gn: torch.Tensor) -> torch.Tensor:
    ...
    return torch.sum((zp_int4_gn.view(G, N // 8, 8) & 0xF) << shifts, dim=2, dtype=torch.int32).contiguous()
...
@pytest.mark.skipif(not on_gfx1x(), reason="Hybrid path is gfx11/gfx12 only")
@pytest.mark.parametrize("group_size,expected_ok", [(32, True), (64, True), (128, True), (256, False)])
...
def test_hip_skinny_wvSplitK_int4_g(dtype, M, K, N, G):
    ...
    scales = (0.05 * torch.rand((N, K // G), device=device, dtype=torch.float32)).to(dtype)
...
def test_rdna_hybrid_w4a16_dispatch(dtype, M, K, N, G):
    ...
    cu_count = num_compute_units()
    out = torch.ops.aphrodite.rdna_hybrid_w4a16_apply(a, b_packed_i8, scales, None, None, cu_count, G)
