...
def _run_emulation_forward(experts, w13_bf16, w2_bf16, hidden_states, topk_weights, topk_ids, E, K, N):
    ...
    output = torch.zeros(hidden_states.shape[0], K, dtype=hidden_states.dtype, device=device)
    experts.apply(
        output=output,
...
def test_gptq_unpack_symmetric(E, K, N, group_size):
    ...
    out = _unpack_and_dequant_int4_gptq(w_packed, scale, None, transpose_output=False, output_dtype=torch.float32)

    assert out.shape == (E, K, N)
    assert torch.allclose(out, ref, atol=0), f"max diff: {(out - ref).abs().max().item()}"
...
def test_gptq_unpack_asymmetric(E, K, N, group_size):
    ...
    out = _unpack_and_dequant_int4_gptq(w_packed, scale, qzeros, transpose_output=False, output_dtype=torch.float32)
...
def test_gptq_unpack_transpose(E, K, N, group_size):
    ...
    assert torch.allclose(out_transposed, out_normal.permute(0, 2, 1).contiguous(), atol=0)
...
def test_awq_unpack_symmetric(E, K, N, group_size):
    ...
    out = _unpack_and_dequant_int4_awq(w_packed, scale, None, transpose_output=False, output_dtype=torch.float32)
...
def test_awq_unpack_asymmetric(E, K, N, group_size):
    ...
    out = _unpack_and_dequant_int4_awq(w_packed, scale, qzeros, transpose_output=False, output_dtype=torch.float32)
...
def test_awq_gptq_unpack_agree(E, K, N, group_size):
    ...
    scale = torch.stack(scale_list).to(device)
    out_gptq = _unpack_and_dequant_int4_gptq(torch.stack(gptq_list).to(device), scale, None, False, torch.float32)
    out_awq = _unpack_and_dequant_int4_awq(torch.stack(awq_list).to(device), scale, None, False, torch.float32)

    assert torch.allclose(out_gptq, out_awq, atol=0), f"max diff: {(out_gptq - out_awq).abs().max().item()}"
...
def test_gptq_process_weights_shapes_and_values(E, K, N, group_size, asym):
    ...
    w13, w13s, w13z, w13_ref, w2, w2s, w2z, w2_ref = _make_gptq_moe_weights(E, K, N, group_size, asym)
...
def test_awq_process_weights_shapes_and_values(E, K, N, group_size, asym):
    ...
    w13, w13s, w13z, w13_ref, w2, w2s, w2z, w2_ref = _make_awq_moe_weights(E, K, N, group_size, asym)
...
def test_gptq_vs_awq_forward_agree(E, K, N, top_k, group_size, num_tokens):
    ...
    experts_gptq = Int4EmulationTritonExperts(moe_config, int4_w4a16_moe_quant_config(dummy_scale, dummy_scale))
    experts_awq = Int4EmulationTritonExperts(moe_config, int4_w4a16_moe_quant_config(dummy_scale, dummy_scale))

    hidden_states = torch.randn(num_tokens, K, dtype=torch.bfloat16, device=device)
    topk_weights = torch.softmax(torch.randn(num_tokens, top_k, dtype=torch.float32, device=device), dim=-1)
    topk_ids = torch.stack([torch.randperm(E, device=device)[:top_k] for _ in range(num_tokens)]).to(torch.int32)

    out_gptq = _run_emulation_forward(experts_gptq, w13_gptq, w2_gptq, hidden_states, topk_weights, topk_ids, E, K, N)
    out_awq = _run_emulation_forward(experts_awq, w13_awq, w2_awq, hidden_states, topk_weights, topk_ids, E, K, N)
...
def test_ep_output_matches_no_ep(E, K, N, top_k, group_size, num_tokens, ep_size, fmt):
    ...
    process_fn = _process_weights_emulation_gptq if fmt == "gptq" else _process_weights_emulation_awq
    res = process_fn(
    ...
    experts_ref = Int4EmulationTritonExperts(moe_config_full, int4_w4a16_moe_quant_config(dummy_scale, dummy_scale))
    out_no_ep = _run_emulation_forward(experts_ref, w13_all, w2_all, hidden_states, topk_weights, topk_ids, E, K, N)
    ...
    for rank in range(ep_size):
        ...
        experts_ep = Int4EmulationTritonExperts(moe_config_ep, int4_w4a16_moe_quant_config(dummy_scale, dummy_scale))
...
def test_ep_gptq_awq_agree(E, K, N, top_k, group_size, num_tokens, ep_size):
    ...
    for rank in range(ep_size):
        ...
        experts_gptq = Int4EmulationTritonExperts(moe_config_ep, int4_w4a16_moe_quant_config(dummy_scale, dummy_scale))
        experts_awq = Int4EmulationTritonExperts(moe_config_ep, int4_w4a16_moe_quant_config(dummy_scale, dummy_scale))
...
@pytest.mark.parametrize("E, K, N, top_k, group_size, num_tokens, ep_size", EP_CONFIGS)
def test_ep_partial_rank_no_active_experts(E, K, N, top_k, group_size, num_tokens, ep_size):
    ...
    assert torch.all(out == 0), f"Expected zeros for inactive rank, got max={out.abs().max().item()}"
...
def test_ep_sum_equals_full_forward(E, K, N, top_k, group_size, num_tokens, ep_size):
    ...
    topk_ids = torch.stack(
        [torch.tensor([(t * top_k + k) % E for k in range(top_k)], dtype=torch.int32) for t in range(num_tokens)]
    ).to(device)
    ...
    experts_full = Int4EmulationTritonExperts(moe_config_full, int4_w4a16_moe_quant_config(dummy_scale, dummy_scale))
    out_full = _run_emulation_forward(experts_full, w13_all, w2_all, hidden_states, topk_weights, topk_ids, E, K, N)
...
@pytest.mark.parametrize("E, K, N, top_k, group_size, num_tokens", E2E_CONFIGS)
@pytest.mark.parametrize("fmt", ["gptq", "awq"])
def test_emulation_output_close_to_bf16_reference(E, K, N, top_k, group_size, num_tokens, fmt):
    ...
    dummy_scale = torch.ones(1, dtype=torch.float16, device=device)
    experts = Int4EmulationTritonExperts(moe_config, int4_w4a16_moe_quant_config(dummy_scale, dummy_scale))

    hidden_states = torch.randn(num_tokens, K, dtype=torch.bfloat16, device=device)
    topk_weights = torch.softmax(torch.randn(num_tokens, top_k, dtype=torch.float32, device=device), dim=-1)
    topk_ids = torch.stack([torch.randperm(E, device=device)[:top_k] for _ in range(num_tokens)]).to(torch.int32)

    out_emulation = _run_emulation_forward(experts, w13_bf16, w2_bf16, hidden_states, topk_weights, topk_ids, E, K, N)
    ...
    rel_l2 = (torch.norm(out_emulation.float() - ref.float()) / torch.norm(ref.float()).clamp(min=1e-6)).item()
