...
# ``expert_mask`` (aiter master ON, ``rocm_aiter_fmoe_enabled``) or Aphrodite's
# -1 index map (master OFF). ``AiterMxfp8Experts.apply`` must forward the right
# 0/1 mask to aiter in BOTH cases. The old code always rebuilt the mask via
...
def _capture_expert_mask(expert_map, *, rocm_aiter_fmoe_enabled, global_num_experts):
    ...
    experts.moe_config = SimpleNamespace(rocm_aiter_fmoe_enabled=rocm_aiter_fmoe_enabled)
...
def test_aiter_mxfp8_ep_expert_mask_both_master_modes():
    ...
    # master OFF: Aphrodite's -1 index map
    ...
    got_off = _capture_expert_mask(idx_map, rocm_aiter_fmoe_enabled=False, global_num_experts=E)
    got_on = _capture_expert_mask(ep_mask, rocm_aiter_fmoe_enabled=True, global_num_experts=E)
