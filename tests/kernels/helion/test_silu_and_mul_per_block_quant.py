...
from aphrodite.kernels.helion.ops.silu_and_mul_per_block_quant import (
    _pick_cache,
    baseline,
    pick_config,
...
if not has_helion():
    pytest.skip(
        "Helion is not installed. Install with: pip install aphrodite[helion]",
        allow_module_level=True,
    )


def _generate_fake_input(num_tokens: int, intermediate_size: int, group_size: int) -> tuple[Any, ...]:
    with FakeTensorMode():
...
class TestSiluAndMulPerBlockQuantConfigPicker:
    ...
    def test_config_picker_exact_match(self):
        ...
        assert selected_key == CaseKey({"intermediate_size": 4096, "group_size": 128, "num_tokens": 16})
    ...
    def test_config_picker_closest_match(self):
        ...
        selected_key = pick_config(args, config_keys)
        assert selected_key == CaseKey({"intermediate_size": 2048, "group_size": 64, "num_tokens": 32})
    ...
    def test_config_picker_fallback_to_largest(self):
        ...
        selected_key = pick_config(args, config_keys)
        assert selected_key == CaseKey({"intermediate_size": 4096, "group_size": 128, "num_tokens": 32})
...
class TestSiluAndMulPerBlockQuantCorrectness:
    ...
    def test_silu_and_mul_per_block_quant(
        ...
        silu_and_mul_per_block_quant(ops_out, x, ops_scales, group_size, scale_ub, is_scale_transposed)
...
class TestSiluAndMulPerBlockQuantIntegration:
    def test_kernel_registration_integration(self):
        from aphrodite.kernels.helion.register import get_registered_kernels
