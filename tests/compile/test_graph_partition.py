...
def test_decompose_size_leaves_scalar_size_with_dim():
    ...
    assert len(remaining) == 1, f"x.size(0) should be left untouched, found {len(remaining)} size nodes"
    assert remaining[0].args == (x, 0), f"size node args changed: {remaining[0].args} (expected (x, 0))"
