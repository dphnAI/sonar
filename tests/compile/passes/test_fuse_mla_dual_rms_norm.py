...
def test_fuse_mla_dual_rms_norm(
    ...
    with aphrodite.config.set_current_aphrodite_config(aphrodite_config), monkeypatch.context() as m:
        ...
        assert fusion_pass.matched_count == 1, f"Expected 1 fused pair, got {fusion_pass.matched_count}"
...
class MLADualRMSNormFp8PerTokenTestModel(torch.nn.Module):
    ...
    Minimal model reproducing the FP8 MLA attention path with per-token quant:
        linear -> split([q_dim, kv_dim])
            +-- q_c -> rocm_aiter_rmsnorm_fused_dynamic_quant -> dequant
    ...
    def forward(self, x: torch.Tensor):
        ...
        q_fp8, q_scale = torch.ops.aphrodite.rocm_aiter_rmsnorm_fused_dynamic_quant(
    ...
    def ops_in_model_before(self):
        return [
            torch.ops.aphrodite.rocm_aiter_rmsnorm_fused_dynamic_quant.default,
            torch.ops.aphrodite_ir.rms_norm.default,
        ]
...
def test_fuse_mla_dual_rms_norm_fp8_per_token(
    ...
    with aphrodite.config.set_current_aphrodite_config(aphrodite_config), monkeypatch.context() as m:
        ...
        assert exact_frac > 0.99, f"q: only {exact_frac:.4f} of elements bit-exact; scales likely differ"
