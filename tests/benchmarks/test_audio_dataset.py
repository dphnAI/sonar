...
import aphrodite.benchmarks.datasets.datasets as datasets_module
import aphrodite.benchmarks.lib.endpoint_request_func as request_func_module
from aphrodite.benchmarks.lib.endpoint_request_func import RequestFuncInput
...
def test_asr_dataset_sample_handles_local_audio_paths(tmp_path: Path) -> None:
    ...
    assert samples[0].prompt == "<|startoftranscript|><|en|><|transcribe|><|notimestamps|>"


@pytest.mark.parametrize("has_filepath", [True, False])
def test_asr_dataset_sample_handles_embedded_audio_bytes(tmp_path: Path, has_filepath: bool) -> None:
    audio_path = tmp_path / "earnings.wav"
    _write_wav(audio_path, duration_s=0.1)
...
def test_async_request_openai_audio_handles_local_audio_paths(
    ...
    output = asyncio.run(request_func_module.async_request_openai_audio(request_input, session))
