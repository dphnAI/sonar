...
def test_mistral_reasoning(
    ...
    if param_dict["content"] is not None:
        ...
        if "[THINK]" in param_dict["output"] and not param_dict["output"].startswith("[THINK]"):
            before_content = param_dict["output"].split("[THINK]")[0]
            before_token_ids = mistral_tokenizer.tokenizer.encode(before_content, bos=False, eos=False)
        ...
        expected_token_ids = before_token_ids + mistral_tokenizer.tokenizer.encode(left_to_encode, bos=False, eos=False)
