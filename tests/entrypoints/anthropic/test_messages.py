...
async def test_anthropic_streaming_cache_usage(client: anthropic.AsyncAnthropic):
    async def get_stream_usage(resp):
        ...
        usage = None
        async for chunk in resp:
            if chunk.type == "message_start" and chunk.message is not None and chunk.message.usage is not None:
    ...
    request = dict(
        ...
        messages=[
            {
                "role": "user",
                "content": "Cache coverage sentinel. " * 256 + "Answer with exactly one word: ok.",
            }
        ],
        stream=True,
    )
