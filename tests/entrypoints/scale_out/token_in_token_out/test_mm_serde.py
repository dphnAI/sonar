...
from aphrodite.entrypoints.scale_out.token_in_token_out.mm_serde import (
...
from aphrodite.inputs import mm_input
...
def _image_engine_input(
    *,
    ...
    item = MultiModalKwargsItem({"pixel_values": pixel_values, "image_grid_thw": image_grid_thw})
...
def test_extract_includes_declared_placeholder_metadata_fields():
    ...
    engine_input, _pixel_values, image_grid_thw = _image_engine_input(grid_keep_on_cpu=False)

    features = extract_mm_features(
        engine_input,
        metadata_fields_for=lambda modality: ({"image_grid_thw"} if modality == "image" else set()),
    )
...
def test_metadata_only_generate_requires_ec_transfer_params():
    ...
    payload = {
        ...
        "features": {
            "mm_hashes": rendered.mm_hashes,
            "mm_placeholders": {
                modality: [p.model_dump() for p in ranges] for modality, ranges in rendered.mm_placeholders.items()
            },
...
def test_metadata_can_replace_or_extend_full_mm_data():
    ...
    full_item = MultiModalKwargsItem({"pixel_values": pixel_elem, "image_grid_thw": metadata_elem})
...
def test_mixed_modalities_preserve_null_metadata_slots():
    image_input, _, _ = _image_engine_input()
    audio_item = MultiModalKwargsItem(
        {"input_features": MultiModalFieldElem(data=torch.ones(2, 3), field=MultiModalBatchedField())}
    )
    engine_input = mm_input(
        prompt_token_ids=[1, 2, 3],
        mm_kwargs={"image": image_input["mm_kwargs"]["image"], "audio": [audio_item]},
        mm_hashes={"image": ["image-hash"], "audio": ["audio-hash"]},
        mm_placeholders={
            "image": [PlaceholderRange(offset=0, length=1)],
            "audio": [PlaceholderRange(offset=1, length=1)],
        },
    )
    ...
    assert features is not None
    assert features.mm_metadata is not None
    assert features.mm_metadata["image"][0] is not None
    assert features.mm_metadata["audio"] == [None]
    restored = MultiModalFeatures.model_validate_json(features.model_dump_json())
    assert restored.mm_metadata == features.mm_metadata
    kwargs = mm_kwargs_from_features(restored)
    assert torch.equal(kwargs["audio"][0]["input_features"].data, audio_item["input_features"].data)
