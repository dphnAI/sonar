...
async def test_namespace_tool_separator(client: openai.AsyncOpenAI, model_name: str):
    ...
    tool_call = next((out for out in response.output if out.type == "function_call"), None)
...
@pytest.mark.asyncio
@pytest.mark.parametrize("model_name", [MODEL_NAME])
async def test_namespace_tool_separator_streaming(client: openai.AsyncOpenAI, model_name: str):
