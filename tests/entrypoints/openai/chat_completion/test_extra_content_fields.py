...
async def test_translategemma_extra_lang_code_fields(client: openai.AsyncOpenAI):
    ...
    assert content is not None and content.strip(), "expected a non-empty Spanish translation"
    ...
    assert any(m in lowered for m in spanish_markers), f"output does not look like Spanish: {content!r}"
...
async def test_translategemma_image_extra_lang_code_fields(
    ...
    content = completion.choices[0].message.content
    assert content is not None and content.strip(), "expected a non-empty translation of the stop-sign image"
    ...
    assert "stop" not in lowered, f"output appears to echo the English source: {content!r}"
