...
def test_memory_profiling():
    ...
    assert abs(total_consumed_ratio - 1) <= 0.05, (
        f"total_consumed={result.total_consumed}, expected={expected_total_consumed}, ratio={total_consumed_ratio}"
    )
    ...
    assert abs(non_kv_ratio - 1) <= 0.05, (
        f"non_kv_cache_memory={result.non_kv_cache_memory}, expected={expected_non_kv}, ratio={non_kv_ratio}"
    )
...
def test_memory_snapshot_uses_psutil_on_integrated_gpu():
    ...
    with (
        patch("aphrodite.utils.mem_utils.current_platform") as mock_platform,
        patch("aphrodite.utils.mem_utils.psutil") as mock_psutil,
