...
class TestMergeInterleavedEmbeddings:
    ...
    def test_image_and_video_mixed(self):
        ...
        input_ids = torch.tensor(tokens)
        is_multimodal = (input_ids == IMAGE_TOKEN_ID) | (input_ids == VIDEO_TOKEN_ID) | (input_ids == AUDIO_TOKEN_ID)
        is_video = is_multimodal & (input_ids == VIDEO_TOKEN_ID)
        is_audio = is_multimodal & (input_ids == AUDIO_TOKEN_ID)
        ...
        assert result[is_image.nonzero(as_tuple=True)[0]].allclose(torch.full((2, hidden), 20.0))
        assert result[is_video.nonzero(as_tuple=True)[0]].allclose(torch.full((2, hidden), 30.0))
        assert result[is_audio.nonzero(as_tuple=True)[0]].allclose(torch.full((2, hidden), 10.0))
