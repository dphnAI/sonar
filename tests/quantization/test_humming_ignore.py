...
from aphrodite.model_executor.layers.quantization.humming import HummingConfig
...
def test_is_layer_skipped_regex_ignore(prefix: str, expected: bool):
    cfg = HummingConfig(full_config={"ignore": K26_IGNORE})
    assert cfg.is_layer_skipped({"ignore": K26_IGNORE}, prefix) is expected
    assert humming_is_layer_skipped({"ignore": K26_IGNORE}, prefix) is expected
...
def test_plain_substring_entries_still_work():
    ...
    assert humming_is_layer_skipped(cfg_dict, "model.vision.encoder.fc") is True
    assert humming_is_layer_skipped(cfg_dict, "model.layers.0.mlp.up_proj") is False
