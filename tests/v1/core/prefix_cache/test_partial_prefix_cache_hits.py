    ...
    *,
    ...
    mamba_group = KVCacheGroupSpec(
        ...
        MambaSpec(
            ...
            prefill_checkpoint_alignment=(16 if num_prefill_checkpoint_blocks > 0 else None),
        ),
    )
...
def test_partial_hit_then_internal_checkpoint_uses_distinct_mamba_blocks():
    ...
    # The producer ends inside its first physical block, so it exports a
    # partial tail. After realignment, the replay exports state@96 with a
    # 64-token offset that satisfies the backend's 16-token alignment.
    ...
    assert manager.allocate_slots(owner, 16, num_computed, computed_blocks) is not None
    manager.free(owner)
    manager.new_step_starts()

    partial_hash = owner.block_hashes[0]
    ...
    realigned_blocks = manager.allocate_slots(replay, 16, num_computed, computed_blocks)
    ...
    replay.num_computed_tokens = 32
    manager.new_step_starts()
    final_blocks = manager.allocate_slots(replay, 68)
    assert final_blocks is not None

    # The skipped physical block precedes the checkpoint and running state.
    assert final_blocks.blocks[1][0].is_null
    _, checkpoint_block_id, running_block_id = final_blocks.get_block_ids()[1]
...
def test_internal_checkpoint_uses_partial_hash_lifecycle():
    ...
    assert drain_boundary_state_offloads(manager) == {request.request_id: [(1, checkpoint_block.block_id, 112)]}
