...
def test_unconditional_to_conditional_rates_basic():
    ...
    assert unconditional_to_conditional_rates([0.9, 0.5, 0.2]) == pytest.approx([0.9, 0.5 / 0.9, 0.2 / 0.5])
...
def test_unconditional_to_conditional_rates_handles_zero():
    ...
    assert unconditional_to_conditional_rates([1.0, 0.6, 0.0, 0.0]) == pytest.approx([1.0, 0.6, 0.0, 0.0])


def test_unconditional_to_conditional_rates_all_ones():
    assert unconditional_to_conditional_rates([1.0, 1.0, 1.0]) == pytest.approx([1.0, 1.0, 1.0])
...
def test_acceptance_length_to_rates(length, n, expected):
    assert SpeculativeConfig._acceptance_length_to_rates(length, n) == pytest.approx(expected)


def test_resolve_length_produces_minvariance_schedule():
    assert SpeculativeConfig._resolve_synthetic_acceptance_rates(3, None, 2.6) == pytest.approx([1.0, 0.6, 0.0])
