...
def _run_batch(sizes: list[int]) -> None:
    ...
    ops.swap_blocks_batch(_addrs(src), _addrs(dst), torch.tensor(sizes, dtype=torch.int64))
    torch.accelerator.synchronize()
    for s, d in zip(src, dst):
        assert torch.equal(d, s)


@pytest.mark.skipif(not current_platform.is_cuda(), reason="swap_blocks_batch requires CUDA")
