...
class TestDrainJobs:
    ...
    def test_logs_warning_after_5s_then_completes(self, monkeypatch):
        ...
        # Spy on the warning logger directly — aphrodite's logger does not
...
class TestBindHostPortDefaults:
    """host/port fall back to APHRODITE_P2P_SIDE_CHANNEL_* when absent."""

    @staticmethod
    def _construct(monkeypatch, dp_index=0, **kwargs) -> P2PSecondaryTierManager:
        """Build a manager with transports/file-mapper stubbed out."""
        monkeypatch.setattr(
            manager_module,
            "FileMapper",
