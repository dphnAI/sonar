...
def _run(
    ...
    full_target = torch.randn(num_logits, VOCAB_SIZE, dtype=torch.bfloat16, device=device)
    ...
    cu_num_logits = torch.arange(0, num_logits + 1, LOGITS_PER_REQ, dtype=torch.int32, device=device)
    ...
    expanded_local_pos = torch.arange(LOGITS_PER_REQ, dtype=torch.int32, device=device).repeat(num_reqs).contiguous()
...
def test_block_verification_i64_indexing(num_reqs: int, max_num_reqs: int, roi_req: int, req_state_idx: int):
    device = torch.device("cuda:0")
    torch.manual_seed(0)
    target_logits = torch.randn(LOGITS_PER_REQ, VOCAB_SIZE, dtype=torch.bfloat16, device=device)
    draft_logits = torch.randn(NUM_SPECULATIVE_STEPS, VOCAB_SIZE, dtype=torch.bfloat16, device=device)
