...
from aphrodite.v1.engine.core import DPEngineCoreProc, EngineCore, EngineCoreProc
...
def _cadenced_dp_engine_core(monkeypatch, results: list[tuple[bool, bool]], dp_sync_interval: int = 32):
...
def test_dp_sync_interval_normal_wave(monkeypatch, dp_sync_interval: int):
    ...
    core, synced = _cadenced_dp_engine_core(monkeypatch, [(True, False)] * 3, dp_sync_interval=dp_sync_interval)
...
def test_dp_sync_interval_idle_pause_consensus_on_first_step(monkeypatch):
    ...
    core, synced = _cadenced_dp_engine_core(monkeypatch, [(False, True)], dp_sync_interval=32)
