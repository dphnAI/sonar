...
DEVICE_BACKENDS: dict[str, DeviceConfig] = {
    "cuda": DeviceConfig(
        available=current_platform.is_cuda() and current_platform.has_device_capability(80),
...
TEST_MODEL = os.getenv("APHRODITE_TEST_MODEL", DEFAULT_MODEL)

# Override backends for MLA models (MLA only supported on CUDA).
if os.getenv("APHRODITE_TEST_MODEL"):
    ...
    if ModelArchConfigConvertorBase(config, config.get_text_config()).is_deepseek_mla():
        DEVICE_BACKENDS["cuda"] = DeviceConfig(
            ...
            backends=["TRITON_MLA"] + (["FLASH_ATTN_MLA"] if flash_attn_supports_mla() else []),
        )
...
BACKENDS: list[str] = sorted({b for cfg in DEVICE_BACKENDS.values() if cfg.available for b in cfg.backends})
