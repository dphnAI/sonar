...
class DummyConfig:
    ...
    vision_config: DummyVisionConfig = dataclasses.field(default_factory=DummyVisionConfig)


def make_video_embedding(t, h, w, interleave_text_tokens: tuple[int, int], video_pruning_rate: float = 0.0):
    """
    ...
    for _ in range(t):
        num_prefix_tokens = random.randint(interleave_text_tokens[0], interleave_text_tokens[1])

        prefix_tokens = random.choices(population, k=num_prefix_tokens)
        vision_tokens = [VISION_START_TOKEN_ID] + [VIDEO_TOKEN_ID] * h * w + [VISION_END_TOKEN_ID]
...
def test_match_qwen3vl_mrope_evs_on(
    ...
    is_video_embed = torch.isin(video_tokens_pruned, torch.tensor([VIDEO_TOKEN_ID], dtype=torch.long))
    ...
    video_embeddings = torch.empty((len(video_tokens_pruned), hidden_size), device=video_mrope.device)
    ...
    computed_mrope[:, 0 : len(prefix_tokens)] = expected_mrope[:, 0 : len(prefix_tokens)]
