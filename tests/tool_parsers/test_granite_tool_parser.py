...
from aphrodite.tool_parsers.granite_tool_parser import GraniteToolParser
...
def test_streaming_parallel_calls_batched_deltas(granite_tokenizer, chunk_size):
    ...
    batched = ["".join(token_deltas[i : i + chunk_size]) for i in range(0, len(token_deltas), chunk_size)]
    reconstructor = run_tool_extraction_streaming(parser, batched, assert_one_tool_per_delta=False)
    ...
    assert json.loads(reconstructor.tool_calls[0].function.arguments) == {"city": "Tokyo"}
