...
def _expand_flashinfer_variants(
    ...
    for backend in all_backends:
        ...
        if "nvfp4" in xqa["kv_cache_dtypes"]:
            xqa["kv_cache_dtypes"] = ", ".join(
                d for d in (d.strip() for d in xqa["kv_cache_dtypes"].split(",")) if d != "nvfp4"
            )
        ...
        trtllm_gen["compute_capability"] = fi_features["trtllm_gen"]["compute_capability"]
